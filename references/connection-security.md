...
公开 Skill 的客户端凭据变量只有 `LETU_KB_API_TOKEN`。若部署方在自己的密钥系统中
使用其他内部名称，必须由部署方在启动 Agent 前显式映射成该变量；Agent 不扫描环境、
不猜别名，也不读取服务端 `.env` 寻找“可能的 Token”。
...
5. 首次使用时向用户显示不含路径、查询和凭据的 `origin`，由用户确认这是其组织提供的 leto AI 服务。若用户在当前任务中已经明确要求使用该精确地址，或明确要求使用当前安全环境中预配置的 `LETU_KB_BASE_URL`，这就是本次任务的确认，不再重复提问；仅仅发现环境变量、历史配置或相似域名不算确认。确认后，本次任务固定使用该 Origin。
